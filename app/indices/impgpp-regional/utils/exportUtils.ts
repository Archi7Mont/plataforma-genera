...
export const exportToPDF = (data: ExportData) => {
  ...
  const addPageIfNeeded = (needed: number) => { if (y + needed > pageHeight - margin) { doc.addPage(); y = margin; } };
  ...
  const boxH = 180; doc.setDrawColor('#D1FAE5'); doc.setFillColor('#F0FDFA');
  doc.roundedRect(margin, y, contentWidth, boxH, 6, 6, 'FD');
  doc.setTextColor('#134E4A'); doc.setFontSize(13); doc.text('Información de Configuración', margin + 16, y + 22);
  ...
  const left = [
    `País: ${data.configuracion.pais}`,
    `Entidad Fiscalizadora: ${data.configuracion.entidadFiscalizadora}`,
    `Institución Auditada: ${data.configuracion.institucionAuditada}`,
  ];
  const right = [
    `Política Pública: ${data.configuracion.politicaPublicaAuditada}`,
    `Año/Período Auditado: ${data.configuracion.anoPeriodoAuditado}`,
  ...
  doc.setTextColor('#115E59'); doc.setFontSize(13); doc.text('Resultados Globales', margin + 16, y + 22);
  ...
  data.resultados.dimensiones.forEach((dim: any) => {
    addPageIfNeeded(40); doc.setFillColor('#14B8A6'); doc.rect(margin, y + 10, contentWidth, 6, 'F');
  ...
  if (noAplica.length) { doc.addPage(); y = margin; doc.setTextColor('#374151'); doc.setFontSize(13); doc.text('Indicadores No Aplica', margin, y); y += 14; noAplica.forEach(it => { writeWrapped(`${it.d} ${it.dn} > ${it.sd} ${it.sdn} > ${it.id}: ${it.t}`, margin, contentWidth, 12, '#4B5563', 10); y += 4; }); }
  ...
  doc.save(`IMP-GPP_Resultados_${data.configuracion.politicaPublicaAuditada.replace(/[^a-zA-Z0-9]/g, '_')}_${new Date().toISOString().slice(0,10)}.pdf`);
};
...
export const exportToExcel = async (data: ExportData) => {
  ...
  worksheet.addRow(['País', data.configuracion.pais]);
  worksheet.addRow(['Entidad Fiscalizadora', data.configuracion.entidadFiscalizadora]);
  worksheet.addRow(['Institución Auditada', data.configuracion.institucionAuditada]);
  worksheet.addRow(['Política Pública Auditada', data.configuracion.politicaPublicaAuditada]);
  worksheet.addRow(['Año/Período Auditado', data.configuracion.anoPeriodoAuditado]);
  ...
  saveAs(blob, `IMP-GPP_Resultados_${data.configuracion.politicaPublicaAuditada.replace(/[^a-zA-Z0-9]/g, '_')}_${new Date().toISOString().slice(0,10)}.xlsx`);
};
